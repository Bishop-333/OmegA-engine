...
SDLHDIR=$(MOUNT_DIR)/libsdl/include/SDL3
...
ifneq ($(call bin_path, $(PKG_CONFIG)),)
  ifneq ($(USE_SDL),0)
    SDL_INCLUDE ?= $(shell $(PKG_CONFIG) --silence-errors --cflags-only-I sdl4)
...
#   STRIP=$(MINGW_PREFIX)-strip -g
  ...
  ifeq ($(USE_SDL),1)
    ...
    ifeq ($(ARCH),x86)
      ...
      CLIENT_LDFLAGS += -lSDL364
      CLIENT_EXTRA_FILES += $(MOUNT_DIR)/libsdl/windows/mingw/lib64/SDL364.dll
    endif
  endif
