...
ifdef MINGW
  ...
  ifeq ($(USE_ZLIB_NG),1)
    BASE_CFLAGS += -I$(ZLIBNGDIR)
    LDFLAGS += $(ZLIBNGDIR)/libz.a
  else
    LDFLAGS += -lz
  endif
...
ifeq ($(USE_CURL),1)
  CURL_CMAKE_ARGS += -G"Unix Makefiles" -DCMAKE_C_COMPILER=$(CC) -DCMAKE_SYSTEM_PROCESSOR=$(ARCH) -DBUILD_CURL_EXE=OFF -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC_LIBS=ON -DCURL_DISABLE_LDAP=ON -DCURL_USE_LIBPSL=OFF -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(CURLDIR)
  ifdef MINGW
    CURL_CMAKE_ARGS += -DCMAKE_SYSTEM_NAME=Windows
  endif
endif
...
ifeq ($(USE_ZLIB_NG),1)
  ZLIBNG_CMAKE_ARGS += -G"Unix Makefiles" -DCMAKE_C_COMPILER=$(CC) -DCMAKE_SYSTEM_PROCESSOR=$(ARCH) -DBUILD_TESTING=OFF -DZLIB_COMPAT=ON -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(ZLIBNGDIR)
  ifdef MINGW
