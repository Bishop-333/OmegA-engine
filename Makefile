...
MODE=$(if $(filter debug,$(MAKECMDGOALS)),debug,release)
TARGETDIR=$(BUILD_DIR)/$(MODE)-$(PLATFORM)-$(ARCH)
ADIR=$(MOUNT_DIR)/asm
...
JPTURBODIR=$(MOUNT_DIR)/thirdparty/libjpeg-turbo
...
ifeq ($(USE_SYSTEM_FLAC),1)
  ...
  FLAC_CFLAGS += -DHAVE_FSEEKO -DHAVE_LROUND -DHAVE_STDINT_H -DHAVE_INTTYPES_H -DFLAC__NO_DLL -DFLAC__HAS_OGG=0
  ifeq ($(ARCH),x86)
    FLAC_CFLAGS += -DFLAC__CPU_IA32
  else
  ifeq ($(ARCH),x86_64)
    FLAC_CFLAGS += -DFLAC__CPU_X86_64
  else
  ifeq ($(ARCH),arm64)
...
ifdef MOD_DEFAULT
  BASE_CFLAGS += -DDEFAULT_GAME=\\\"$(MOD_DEFAULT)\\\"
endif
...
ifeq ($(USE_JPEG_TURBO),1)
  JPTURBO_CMAKE_ARGS += $(CMAKE_ARGS) -DCMAKE_POLICY_DEFAULT_CMP0219=NEW -DENABLE_SHARED=OFF -DWITH_TOOLS=OFF -DWITH_TESTS=OFF -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(TARGETDIR)/libjpeg-turbo
  ifeq ($(COMPILE_PLATFORM),darwin)
    ifeq ($(ARCH),x86_64)
...
ifeq ($(USE_CURL),1)
  ifneq ($(USE_CURL_DLOPEN),1)
    ifdef MINGW
      CURL_CMAKE_ARGS += $(CMAKE_ARGS) -DBUILD_CURL_EXE=OFF -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC_LIBS=ON -DCURL_DISABLE_LDAP=ON -DCURL_USE_LIBPSL=OFF -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(TARGETDIR)/libcurl
      ifeq ($(ARCH),x86)
...
debug:
	@$(MAKE) targets B=$(BD) MODE=debug CFLAGS="$(CFLAGS) $(DEBUG_CFLAGS)" LDFLAGS="$(LDFLAGS) $(DEBUG_LDFLAGS)" V=$(V) NO_STRIP=1

release:
	@$(MAKE) targets B=$(BR) MODE=release CFLAGS="$(CFLAGS) $(RELEASE_CFLAGS)" V=$(V)
...
targets: makedirs tools thirdparty
	@echo ""
...
$(B)/$(TARGET_REND1): $(Q3REND1OBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) -o $@ $(Q3REND1OBJ) $(SHLIBCFLAGS) $(SHLIBLDFLAGS) $(CLIENT_LDFLAGS)

$(B)/$(TARGET_RENDV): $(Q3RENDVOBJ)
	$(echo_cmd) "LD $@"
	$(Q)$(CC) -o $@ $(Q3RENDVOBJ) $(SHLIBCFLAGS) $(SHLIBLDFLAGS) $(CLIENT_LDFLAGS)
