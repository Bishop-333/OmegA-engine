...
ifeq ($(USE_CURL),1)
  ...
  ifeq ($(USE_CURL_DLOPEN),1)
    ...
    ifeq ($(MINGW),1)
      ...
      CURL_CMAKE_ARGS += -G"Unix Makefiles" -DCMAKE_C_COMPILER=$(CC) -DCMAKE_SYSTEM_PROCESSOR=$(ARCH) -DBUILD_CURL_EXE=OFF -DBUILD_SHARED_LIBS=OFF -DBUILD_STATIC_LIBS=ON -DCURL_DISABLE_LDAP=ON -DCURL_USE_LIBPSL=OFF -DCMAKE_SYSTEM_NAME=Windows -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(TARGETDIR)/libcurl
      ifeq ($(ARCH),x86)
...
ifeq ($(USE_OPENAL),1)
  ...
  OPENAL_CMAKE_ARGS += -G"Unix Makefiles" -DCMAKE_C_COMPILER=$(CC) -DCMAKE_CXX_COMPILER=$(CXX) -DCMAKE_SYSTEM_PROCESSOR=$(ARCH) -DLIBTYPE=STATIC -DALSOFT_UTILS=OFF -DALSOFT_EXAMPLES=OFF -DCMAKE_INSTALL_PREFIX=$(CURDIR)/$(TARGETDIR)/libopenal
  ifdef MINGW
    OPENAL_CMAKE_ARGS += -DCMAKE_SYSTEM_NAME=Windows
  endif
  ifeq ($(COMPILE_PLATFORM),darwin)
    OPENAL_CMAKE_ARGS += -DCMAKE_OSX_ARCHITECTURES=$(ARCH) -DCMAKE_OSX_DEPLOYMENT_TARGET=$(MACOSX_VERSION_MIN) -DCMAKE_CXX_FLAGS="-Wno-deprecated-anon-enum-enum-conversion"
  endif
endif
