...
static qboolean	com_deferredConfig = qfalse;
...
===============
*/
void Com_WriteConfigSnapshot( void ) {
	fileHandle_t f;

	f = FS_FOpenFileWrite( "q3config.cfg.tmp" );
	if ( f == FS_INVALID_HANDLE ) {
		return;
	}

	FS_Printf( f, "// copy of the latest snapshot of your q3config.cfg, generated by OmegA engine, do not modify\n" );
	Cvar_WriteVariables( f );
	FS_FCloseFile( f );

	com_deferredConfig = qtrue;
}


/*
===============
Com_RestoreConfigSnapshot
===============
*/
static void Com_RestoreConfigSnapshot( void ) {
	Cbuf_ExecuteText( EXEC_NOW, "exec q3config.cfg.tmp\n" );
	Cbuf_ExecuteText( EXEC_APPEND, "vid_restart\n" );
	FS_HomeRemove( "q3config.cfg.tmp" );

	com_deferredConfig = qfalse;
}
...
void Com_Frame( qboolean noDelay ) {
	...
	// restore latest snapshot of q3config.cfg after mod switch
	if ( com_deferredConfig ) {
		Com_RestoreConfigSnapshot();
	}
