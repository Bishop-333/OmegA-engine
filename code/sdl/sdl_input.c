...
static keyNum_t IN_TranslateSDLToQ3Key( const SDL_KeyboardEvent *keyEvent, qboolean down )
{
	...
	{
		if ( keyEvent->scancode >= SDL_SCANCODE_A && keyEvent->scancode <= SDL_SCANCODE_Z )
		{
			key = 'a' + keyEvent->scancode - SDL_SCANCODE_A;
		}
		else
		{
	...
	if( !key && keyEvent->key >= SDLK_SPACE && keyEvent->key < SDLK_DELETE )
	{
